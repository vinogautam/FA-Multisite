...
class VisaTable extends WP_List_Table {
    ...
    function column_default($item, $column_name){
        switch($column_name){
            case 'endorser_id':
                ...
                return $user_info->user_email;
            case 'status':
                return $item[$column_name] ? "Gift sent" : "New Request";
            case 'action':    
                return $item['status'] ? '-' : '<a href="?action=change_status&id='.$item['id'].'">Gift sent manually</a>';
            case 'endorser_id':
                return get_user_meta($item[$column_name], 'first_name', true).' '.get_user_meta($item[$column_name], 'last_name', true);
            case 'transaction_on':
                return date('Y/m/d H:i', strtotime($item[$column_name]));
    ...
    function prepare_items() {
        ...
        $data = objectToArray($wpdb->get_results("select * from visa_transaction"));
        $newdat = array();
        foreach($data as $v){
            $newdat[] = $v;
        }
...
class RequestTable extends WP_List_Table {
    ...
    function process_bulk_action() {
        ...
        $del_val = $_REQUEST['movie'];
        foreach($del_val as $val) {
            $wpdb->delete($wpdb->prefix . "points_transaction", array( 'id' => $val ) );
        }}
